...
impl MemoryManager {
    ...
    fn validate_core_read(filename: &str) -> Result<()> {
        if CORE_FILES.contains(&filename) { Ok(()) }
        else { bail!("Invalid core memory file: {filename}") }
    }

    fn validate_core_write(filename: &str) -> Result<()> {
        if WRITABLE_CORE_FILES.contains(&filename) { Ok(()) }
        else { bail!("Cannot write to core memory file: {filename}") }
    }
    ...
    pub fn search_logs(&self, query: &str) -> Result<Vec<String>> {
        ...
        if !logs_dir.exists() { return Ok(results); }
